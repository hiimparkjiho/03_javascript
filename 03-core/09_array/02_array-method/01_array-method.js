...
console.log(`foodList.lastIndexOf('삼겹살') : ${foodList.lastIndexOf('삼겹살')}`);      // -1   // 없는 값으로 -1 출력

console.log(`foodList.includes('물회') : ${foodList.includes('물회')}`);                // true // 값이 있으냐?
...
const mix = idol1.concat(idol2);        //합친 것
...
// slice(시작인덱스, 종료인덱스)
// slice (start(인덱스의 시작 값), end(인덱스의 마지막 값)) end: 요소의 값은 반환하지 않는다.
console.log(`front.slice(1, 3) : ${front.slice(1, 3)}`);                        // CSS,JavaScript   (자르는 것)
